...
class TestEmbeddingModel:
    ...
    def test_embed_documents_returns_list_of_vectors(self, mock_hf_model):
        import numpy as np
        from rag.embeddings import EmbeddingModel
        mock_hf_model.encode.return_value = np.array([
            [0.1, 0.2, 0.3, 0.4],
            [0.1, 0.2, 0.3, 0.4],
            [0.1, 0.2, 0.3, 0.4]
        ], dtype="float32")
...
class TestRAGPipeline:
    ...
    def mock_llm(self):
        ...
        # Add OpenAI-style nested client interface
        llm.chat = MagicMock()
    ...
    def test_query_expanded_query_contains_mood_descriptors(self, pipeline):
        result = pipeline.query("something warm", mood="cozy")
        eq = result["expanded_query"].lower()
        # Should contain the original query
        assert "something warm" in eq
        
    def test_query_passes_history_to_prompt_builder(self, pipeline, mock_llm):
        # 1. Clear call histories
        mock_llm.complete.reset_mock()
        
        # 2. Build the nested mock structure to satisfy self._llm_client.chat.completions.create()
        mock_response = MagicMock()
        mock_response.choices[0].message.content = LLM_FULL_RESPONSE
        
        # Set up BOTH interfaces so it doesn't matter which path the pipeline takes
        mock_llm.complete.return_value = LLM_FULL_RESPONSE
        mock_llm.chat = MagicMock()
        mock_llm.chat.completions = MagicMock()
        mock_llm.chat.completions.create.return_value = mock_response
        
        # 3. Ensure your pipeline points completely to this fixture instance
        ...
        # 5. Verify the call payload
        # If your pipeline calls ._call_openai directly, check chat.completions.create
        if mock_llm.chat.completions.create.called:
            mock_llm.chat.completions.create.assert_called_once()
            call_args, kwargs = mock_llm.chat.completions.create.call_args
            messages = kwargs.get("messages")
        else:
            # If it uses your unified complete implementation
            mock_llm.complete.assert_called_once()
            call_args, kwargs = mock_llm.complete.call_args
            messages = kwargs.get("messages") or (call_args[0] if call_args else None)
    ...
    def test_query_latency_is_positive(self, pipeline):
        ...
        mock_llm =  MagicMock()
        mock_response = MagicMock()
        mock_response.choices[0].message.content = "Mocked Healthy Response"
        mock_llm.chat.completions.create.return_value = mock_response
